...
export default function ProductsPage() {
  ...
  const list = useQuery({
    queryKey: ["products", q, page, pageSize],
    queryFn: () => api.get<ProductList>(`/products?q=${encodeURIComponent(q)}&page=${page}&pageSize=${pageSize}`),
  });
    ...
    <div style={{ display: "grid", gap: 16 }}>
      ...
      {list.data && list.data.total > 0 && (
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginTop: 8 }}>
          <span>
            Página {list.data.page} de {Math.ceil(list.data.total / list.data.pageSize)} • {list.data.total} resultados
          </span>
